        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add" >
                <!--  
                  以前的路由跳转，是只从A路由跳转到B路由，
                  而这里在加入购物车、进行路由跳转之前，发请求把要购买的产品及数量通过请求的方式通过服务器，服务器进行相应的存储
                -->
  ...
  export default {
    ...
    methods: {
      ...
      changeActive(saleAttrValue, arr){
        // 这里运用的是排他思想：即容器里其他的全为0，只有被点击的为1
        // 所以需要获取两个参数，第一个参数是被点击的元素，另一个是其所在的容器
        arr.forEach(item => {
          item.isChecked = '0';
        });
      ...
      changeSkuNum(event){
        // 用户输入的文本 * 1
        // 字符串 * 1 得到的是NaN，所以可以判断结果是否为NaN来知晓用户输入的是否是字符串（字符串非法）
      ...
      async addShopcart(){
        // 发请求---将产品加入到数据库（通知服务器）
        /*
          当前这里派发一个action，向服务器发请求，判断加入购物车是成功还是失败了，进行相应的操作
          this.$store.dispatch('addOrUpdateShopCart', {skuId: this.$route.params.skuId, skuNum: this.skuNum})
          上面这行代码实质就是调用仓库中的addOrUpdateShopCart函数
          这个方法加上了async，所以返回值一定是Promise对象，要么成功，要么失败

        */
        
        try {
          await this.$store.dispatch('addOrUpdateShopCart', {skuId: this.$route.params.skuId, skuNum: this.skuNum})
          // 路由跳转(还需要将产品的信息带给addcartsuccess路由组件)
          // 可以采用下面的query参数写法传递数据，但不推荐
          // this.$router.push({name: 'addcartsuccess', query: {skuInfo: this.skuInfo, skuNum: this.skuNum}});
          // 一些简单的数据skuNum，可以通过query形式给路由组件传递过去
          // 产品信息的数据是对象，是比较复杂的数据，可以采用会话存储的方式（不持久化，会话结束数据就消失）
          // 本地存储和会话存储的数据是字符串，所以如果存储对象，需要转换为JSON
